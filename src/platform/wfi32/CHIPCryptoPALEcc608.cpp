...
CHIP_ERROR ECC608_Set_Slot(int slot_id)
{
    gkey_slot = slot_id;
...
CHIP_ERROR P256Keypair::ECDSA_sign_msg(const uint8_t * msg, const size_t msg_length, P256ECDSASignature & out_signature) const
{
    ...
    ChipLogError(Crypto, "P256Keypair::ECDSA_sign_msg");
...
CHIP_ERROR P256Keypair::ECDH_derive_secret(const P256PublicKey & remote_public_key, P256ECDHDerivedSecret & out_secret) const
{
    ...
    ChipLogError(Crypto, "ECDH_derive_secret In");
    CHIP_ERROR error     = CHIP_NO_ERROR;
    int result           = 0;
...
CHIP_ERROR P256Keypair::Initialize(ECPKeyTarget key_target)
{
    ...
    uint8_t pubkey[64];
    atcab_genkey_base(0x08, gkey_slot, NULL, pubkey);

    atca_mbedtls_pk_init(&pk, gkey_slot);
    
    /* move to next slot, only 0, 2, 3, 4 contain private key for cryptographic primitives*/
    if (gkey_slot == 0)
        gkey_slot = 2;
    else if (gkey_slot < 4)
        gkey_slot ++;        

    memcpy(keypair, mbedtls_pk_ec(pk), sizeof(mbedtls_ecp_keypair));
    //ChipLogProgress(Crypto, "[%s] keypair->d.p = %d, keypair->d.n = %d, keypair->d.s = %d", __func__, *(uint16_t*) keypair->d.p, keypair->d.n, keypair->d.s);
#else
