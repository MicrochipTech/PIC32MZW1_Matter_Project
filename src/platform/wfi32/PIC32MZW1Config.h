 ...
 */
class PIC32MZW1Config
{
public:
    struct Key;

    // Maximum length of a key name
    static constexpr size_t kMaxConfigKeyNameLength = 200;//MTB_KVSTORE_MAX_KEY_SIZE;
...
inline CHIP_ERROR PIC32MZW1Config::Key::to_str(char * buf, size_t buf_size) const
{
    if (buf_size < len() + 1)
    {
...
inline size_t PIC32MZW1Config::Key::len() const
{
    // + 1 for separating ';'
...
inline bool PIC32MZW1Config::Key::operator==(const Key & other) const
{
