...
CHIP_ERROR LightingManager::Init()
{
    ...
    if (sLightTimer == NULL)
    {
        PIC32_LOG("sLockTimer timer create failed");
...
bool LightingManager::InitiateAction(int32_t aActor, Action_t aAction)
{
    ...
    //    light->mState == kState_ONInitiated            
    //else if (aAction == OFF_ACTION)
    //    light->mState == kState_OFFInitiated
...
void LightingManager::StartTimer(uint32_t aTimeoutMs)
{
    ...
    {
        PIC32_LOG("sLockTimer timer start() failed");
...
void LightingManager::CancelTimer(void)
{
    ...
    {
        PIC32_LOG("sLightTimer stop() failed");
...
void LightingManager::AutoTurnOffTimerEventHandler(AppEvent * aEvent)
{
    ...
    PIC32_LOG("Auto Turn Off has been triggered!");
...
void LightingManager::ActuatorMovementTimerEventHandler(AppEvent * aEvent)
{
    ...
    if (actionCompleted != INVALID_ACTION)
    {
        ...
        if (light->mAutoTurnOff && actionCompleted == ON_ACTION)
        {
            ...
            PIC32_LOG("Auto Turn off enabled. Will be triggered in %u seconds", light->mAutoTurnOffDuration);
        }
    }
}
