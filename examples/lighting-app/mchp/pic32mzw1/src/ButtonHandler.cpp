...
{
    bool result;
    result = GPIO_PinInterruptCallbackRegister( GPIO_PIN_RB8, light_button_callback, (uintptr_t) NULL);
    if (!result)
        PIC32_LOG("Light button interrupt set fail..");
    GPIO_PinIntEnable(GPIO_PIN_RB8, GPIO_INTERRUPT_ON_MISMATCH);
    
    result = GPIO_PinInterruptCallbackRegister( GPIO_PIN_RA10, func_button_callback, (uintptr_t) NULL);
    if (!result)
